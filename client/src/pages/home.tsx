...
const MODEL_META: Record<string, { icon: string; label: string }> = {
  ...
  Kimi: {
    icon: "https://upload.wikimedia.org/wikipedia/en/thumb/8/87/Kimi-logo-2025.png/250px-Kimi-logo-2025.png",
    label: "2.6",
  },
  DeepSeek: {
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/20 selection:text-primary">
      {/* Navigation */}
      <nav className={`fixed top-0 w-full z-50 transition-all duration-200 border-b ${isScrolled ? "dark:bg-black/80 bg-white/80 backdrop-blur-md border-border" : "bg-transparent border-transparent"}`}>
  ...
  {theme === "dark" ? <Sun className="w-5 h-5" /> : <Moon className="w-5 h-5" />}
      ...
      <main className="pt-24 pb-0">
       ...
       {/*    <div className="mb-8 flex justify-center">
          ...
          <Card className="max-w-xl mx-auto border border-border shadow-sm p-6 mb-8 bg-card backdrop-blur-sm text-center">
...
<section className="border-y border-[#c6c6c5] bg-[#fcfdf7] dark:border-border dark:bg-secondary/50 py-4">
      ...
      Startup (P26)
                ...
                <tbody>
                  {LEADERBOARD_DATA.map((row, i) => (
                    <tr key={row.name as string} className={`border-b border-border/50 hover:bg-secondary/20 transition-colors ${i % 2 === 0 ? 'bg-card' : 'bg-secondary/10'}`}>
                      ...
                      {["Claude","GPT","Gemini","Grok","GLM","Kimi","DeepSeek","Qwen","MiniMax","Mimo"].map(m => {
                        ...
                        const cls = v === 0 ? 'text-muted-foreground/30' : v >= 80 ? 'text-green-700 dark:text-green-400 font-semibold' : v >= 40 ? 'text-foreground' : 'text-muted-foreground';
                        return (
        ...
        <section id="how-it-works" className="py-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            {[
              ...
              <div key={i} className="relative bg-card pt-8 px-6 pb-8 border border-border rounded-xl shadow-sm hover:shadow-md transition-shadow yc-shadow text-center group">
        ...
        <section className="py-24 bg-zinc-950 dark:bg-zinc-900">
        ...
        <section id="protocol" className="py-24 px-4 sm:px-6 lg:px-8 max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
            ...
            <div className="bg-secondary/50 p-8 rounded-2xl border border-border">
              ...
              <div className="mt-8 p-4 bg-background rounded-lg text-sm font-medium border border-border text-foreground">
          ...
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              ...
              <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
                YC Bench is open. Join the community and help develop the benchmark.
              </p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              {[
                ...
                <div key={i} className="bg-card border border-border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow flex gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recentPosts.slice(0, 3).map((post) => (
                <Link key={post.slug} href={`/blog/${post.slug}`}>
                  <article className="group bg-card border border-border rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow h-full flex flex-col cursor-pointer">
